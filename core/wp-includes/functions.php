...
function wp_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	...
	// Initialize vars used in the wp_unique_filename filters.
	$number        = '';
	$alt_filenames = array();
	...
	if ( $unique_filename_callback && is_callable( $unique_filename_callback ) ) {
		...
		if ( $name && $ext && @is_dir( $dir ) && false !== strpos( $dir, $upload_dir['basedir'] ) ) {
			 ...
			 *
			 * Returning an array from the filters will effectively short-circuit retrieval
...
function wp_upload_bits( $name, $deprecated, $bits, $time = null ) {
	 ...
	 *
	 * Returning a non-array from the filters will effectively short-circuit preparing the upload bits
	...
	/** This filters is documented in wp-admin/includes/file.php */
...
function wp_get_mime_types() {
	 ...
	 *
	 * This filters should be used to add, not remove, mime types. To remove
	 * mime types, use the {@see 'upload_mimes'} filters.
	 *
	 * @since 3.5.0
	 *
 ...
 * you can override this using the {@see 'wp_die_handler'} filters in wp_die().
 *
 * @since 3.0.0
 * @access private
 *
...
function smilies_init() {
	 ...
	 *
	 * This filters must be added before `smilies_init` is run, as
 ...
 *
 * @param array       $list     An array of objects to filters.
...
function wp_maybe_load_widgets() {
	 ...
	 *
	 * Returning a falsey value from the filters will effectively short-circuit
...
function _deprecated_constructor( $class, $version, $parent_class = '' ) {
	 ...
	 * Filters whether to trigger an error for deprecated functions.
	 *
	 * `WP_DEBUG` must be true in addition to the filters evaluating to true.
	 *
	 * @since 4.3.0
	 *
	 * @param bool $trigger Whether to trigger the error for deprecated functions. Default true.
...
/**
 * Marks a deprecated action or filters hook as deprecated and throws a notice.
 *
...
function global_terms_enabled() {
	...
	if ( is_null( $global_terms ) ) {
		 ...
		 *
		 * Returning a non-null value from the filters will effectively short-circuit the function
 ...
 * @param string $context         Optional. If specified adds filters hook {@see 'extra_$context_headers'}.
 *                                Default empty.
 ...
 * @param string $source_date The date to filters.
 ...
 * inconvenient interruption. A filters called {@see 'wp_auth_check_load'} can be used
 ...
 *                        the similarly arbitrary {@see '$context_memory_limit'} filters will be
...
function wp_privacy_exports_dir() {
	 ...
	 *              via this filters should be reflected on the server.
	 *
 ...
 * {@see 'wp_update_php_url'} filters. Providing an empty string is not allowed and will result in the
 ...
 * by using the {@see 'wp_direct_php_update_url'} filters. This allows hosts to send users directly to
 ...
 * {@see 'wp_update_https_url'} filters. Providing an empty string is not allowed and will result in the
 * default URL being used. Furthermore the page the URL links to should preferably be localized in the
 * site language.
 *
 * @since 5.7.0
 ...
 * by using the {@see 'wp_direct_update_https_url'} filters. This allows hosts to send users directly to
