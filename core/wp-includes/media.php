 ...
 * not set.
 *
 * Finally, there is a filters named {@see 'editor_max_image_size'}, that will be
 ...
 *
 * A plugin may use the {@see 'image_downsize'} filters to hook into and offer image
...
function image_downsize( $id, $size = 'medium' ) {
	 ...
	 *
	 * Returning a truthy value from the filters will effectively short-circuit
 ...
 *
 * The {@see 'get_image_tag_class'} filters allows for changing the class name for the
 * image without having to use regular expressions on the HTML content. The
 * parameters are: what WordPress will use for the class, the Attachment ID,
 * image align value, and the size the image should be.
 *
 * The second filters, {@see 'get_image_tag'}, has the HTML content, which can then be
...
function image_resize_dimensions( $orig_w, $orig_h, $dest_w, $dest_h, $crop = false ) {
	 ...
	 *
	 * Returning a non-null value from the filters will effectively short-circuit
 ...
 * {@see 'image_make_intermediate_size'} filters can be used to hook in and change the
...
function wp_get_attachment_image_src( $attachment_id, $size = 'thumbnail', $icon = false ) {
	...
	if ( ! $image ) {
		...
		if ( $icon ) {
			...
			if ( $src ) {
				/** This filters is documented in wp-includes/post.php */
...
function wp_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	/**
	 * Let plugins pre-filters the image meta to be able to fix inconsistencies in the stored data.
	 *
	 * @since 4.5.0
	 *
	 * @param array  $image_meta    The image meta data as returned by 'wp_get_attachment_metadata()'.
	 ...
	 * contain a unique hash. Look for that hash and use it later to filters
 ...
 * @param string $context  Additional context, like the current filters name
 ...
 * action hooks to dynamically add/remove itself so as to only filters post thumbnails.
 *
 * @ignore
 * @since 2.9.0
 *
 ...
 * filters hook. Internal use only.
 *
 * @ignore
 * @since 2.9.0
 *
 ...
 * filters is {@see 'img_caption_shortcode'} and passes an empty string, the attr
...
function wp_playlist_shortcode( $attr ) {
	 ...
	 *
	 * Returning a non-empty value from the filters will short-circuit generation
...
function wp_prepare_attachment_for_js( $attachment ) {
	...
	if ( $meta && ( 'image' === $type || ! empty( $meta['sizes'] ) ) ) {
		...
		/** This filters is documented in wp-admin/includes/media.php */
		 ...
		 * First: run the image_downsize filters. If it returns something, we can use its data.
		 * If the filters does not return something, then image_downsize() is just an expensive way
		 * to check the image metadata, which we do second.
		 */
		foreach ( $possible_sizes as $size => $label ) {

			/** This filters is documented in wp-includes/media.php */
			...
			if ( $downsize ) {
				...
				// Nothing from the filters, so consult image metadata if we have it.
...
function wp_enqueue_media( $args = array() ) {
	 ...
	 * the media library.  If this filters returns `null`, a query will be run
	 * to determine whether the media library contains any audio items.  This
	 * was the default behavior prior to version 4.8.0, but this query is
	 * expensive for large media libraries.
	 *
	 * @since 4.7.4
	 * @since 4.8.0 The filters's default value is `true` rather than `null`.
	 *
	 ...
	 *
	 * By default (if this filters does not return an array), a query will be
