...
function wp_save_post_revision( $post_id ) {
	...
	if ( $revisions ) {
		 ...
		 * This filters can override that so a revision is saved even if nothing has changed.
		 *
		 * @since 3.6.0
		 *
		...
		if ( isset( $last_revision ) && apply_filters( 'wp_save_post_revision_check_for_changes', true, $last_revision, $post ) ) {
			 ...
			 *
			 * By default a revision is saved only if one of the revisioned fields has changed.
			 * This filters allows for additional checks to determine if there were changes.
			 *
			 * @since 4.1.0
			 *
 ...
 * @param string      $filter Optional sanitation filters. See sanitize_post().
...
function wp_revisions_to_keep( $post ) {
	 ...
	 *
	 * Overrides both the value of WP_POST_REVISIONS and the {@see 'wp_revisions_to_keep'} filters.
	 *
