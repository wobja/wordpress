...
function receiveEntityRecords(kind, name, records, query, invalidateCache = false, edits) {
  // Auto drafts should not have titles, but some plugins rely on them so we can't filters this
  // on the server.
  if (kind === 'postType') {
...
function* resolvers_experimentalGetTemplateForLink(link) {
  ...
  // We could potentially do so by adding a "filters" to the `wp_template` end point.
