 ...
 * @param {Object}          state      Blocks state.
 * @param {(string|Object)} nameOrType Block name or type object.
 * @param {string}          searchTerm Search term by which to filters.
 *
...
function registerBlockType(blockNameOrMetadata, settings) {
  ...
  if (settings.deprecated) {
    ...
    Object(external_wp_hooks_["applyFilters"])('blocks.registerBlockType', // Merge deprecation keys with pre-filters settings
...
const getBlockTypesForPossibleFromTransforms = blocks => {
  ...
  const allBlockTypes = registration_getBlockTypes(); // filters all blocks to find those with a 'from' transform.
...
const getBlockTypesForPossibleToTransforms = blocks => {
  ...
  const transformsTo = getBlockTransforms('to', blockType.name); // filters all 'to' transforms to find those that are possible.
 ...
 *     .filters( ( tr ) => tr.lastChild.textContent.indexOf( 'Boolean attribute' ) !== -1 )
 ...
 *
 * Object.keys( Array.from( document.querySelectorAll( '#attributes-1 > tbody > tr' ) )
 *     .filters( ( tr ) => /^("(.+?)";?\s*)+/.test( tr.lastChild.textContent.trim() ) )
 *     .reduce( ( result, tr ) => Object.assign( result, {
 *         [ tr.firstChild.textContent.trim() ]: true
 *     } ), {} ) ).sort();
 *
...
/**
 * This filters takes figure content out of paragraphs, wraps it in a figure
 * element, and moves any anchors with it if needed.
 *
 * @param {Node}     node   The node to filters.
 * @param {Document} doc    The document of the node.
 * @param {Object}   schema The schema to use.
 *
 * @return {void}
 ...
 *
 * @param {NodeList} nodeList The nodeList to filters.
 ...
 *
 * @param {string} HTML    The HTML to filters.
 * @param {Array}  filters An array of functions that can mutate with the provided node.
 * @param {Object} schema  The schema to use.
 *
  ...
  for (var i = 0; i < extension.length; ++i) {
    ...
    if (type === 'listener') {
      ...
      if (showdown.helper.isUndefined(ext.filter) && showdown.helper.isUndefined(ext.regex)) {
        ret.valid = false;
        ret.error = baseMsg + type + ' extensions must define either a "regex" property or a "filters" method';
        return ret;
      }
    }
    ...
    if (ext.filter) {
      if (typeof ext.filter !== 'function') {
        ret.valid = false;
        ret.error = baseMsg + '"filters" must be a function, but ' + typeof ext.filter + ' given';
        return ret;
      }
    } else if (ext.regex) {
