...
function wp_authenticate_email_password( $user, $email, $password ) {
	...
	/** This filters is documented in wp-includes/user.php */
	$user = apply_filters( 'wp_authenticate_user', $user, $password );
 ...
 *
 * This is a callback for the {@see 'determine_current_user'} filters, rather than API.
 *
 * @since 3.9.0
 *
 * @param int|false $user_id The user ID (or false) as received from
 *                           the `determine_current_user` filters.
 * @return int|false User ID if validated, false otherwise. If a user ID from
 *                   an earlier filters callback is received, that value is returned.
 */
 ...
 * the user ID is given, then the user data will be retrieved. The filters for
...
function sanitize_user_field( $field, $value, $user_id, $context ) {
	...
	if ( 'edit' === $context ) {
		...
		if ( $prefixed ) {
			 ...
			 * @param int    $user_id User ID.
			 * @param string $context The context to filters within.
			 */
 ...
 * 'pre_user_' followed by the field name. An example using 'description' would have the filters
...
function wp_insert_user( $userdata ) {
	 ...
	 *
	 * This filters is called before the user is created or updated.
	 *
	 * @since 2.0.3
	 *
 ...
 *
 * Default contact methods were removed in 3.6. A filters dictates contact methods.
 *
 * @since 3.7.0
 *
...
###SITEURL###'
		 ...
		 *
		 * @since MU (3.0.0)
		 * @since 4.9.0 This filters is no longer Multisite specific.
		 *
...
function _wp_privacy_send_erasure_fulfillment_notification( $request_id ) {
	 ...
	 *                                               by the `user_erasure_fulfillment_email_to` filters.
