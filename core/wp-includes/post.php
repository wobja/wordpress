 ...
 *
 * By default the path will go through the 'get_attached_file' filters, but
 ...
 * attached filename through a filters.
 *
 * @since 2.0.0
 *
 ...
 *
 * See sanitize_post() for optional $filters values. Also, the parameter
 ...
 *                                 correspond to a WP_Post object, an associative array, or a numeric array,
 *                                 respectively. Default OBJECT.
 * @param string           $filter Optional. Type of filters to apply. Accepts 'raw', 'edit', 'db',
 ...
 *
 * The context values are based off of the taxonomy filters functions and
 ...
 * @param string      $context Optional. How to filters the field. Accepts 'raw', 'edit', 'db',
...
function get_post_status( $post = null ) {
	 ...
	 *
	 * @since 4.4.0
	 * @since 5.7.0 The attachment post type is now passed through this filters.
	 *
 ...
 * - `uploaded_to_this_item` - Label for the media frame filters. Default is 'Uploaded to this post' /
 ...
 *                       'Filter pages list'.
 * - `filter_by_date` - Label for the date filters in list tables. Default is 'Filter by date'.
 ...
 *
 * Note: To set labels used in post type admin notices, see the {@see 'post_updated_messages'} filters.
 *
 * @since 3.0.0
...
function sanitize_post( $post, $context = 'display' ) {
	if ( is_object( $post ) ) {
		...
		// Check if post already filtered for this context.
		if ( isset( $post['filters'] ) && $context == $post['filters'] ) {
			return $post;
		}
		if ( ! isset( $post['ID'] ) ) {
			$post['ID'] = 0;
...
function wp_insert_post( $postarr, $wp_error = false, $fire_after_hooks = true ) {
	...
	/*
	 * The 'wp_insert_post_parent' filters expects all variables to be present.
	...
	$new_postarr = array_merge(
		...
		compact( array_diff( array_keys( $defaults ), array( 'context', 'filters' ) ) )
	);
...
function get_lastpostmodified( $timezone = 'server', $post_type = 'any' ) {
	/**
	 * Pre-filters the return value of get_lastpostmodified() before the query is run.
	 *
	 * @since 4.4.0
	 *
 ...
 *                            wp_transition_post_status() and the default filters for _future_post_hook().
 * @param WP_Post $post       Post object.
 */
 ...
 * to the {@see 'wp_insert_post_parent'} filters.
 *
 * @since 3.1.0
 *
 ...
 *
 * This can be used as a callback on the `wp_untrash_post_status` filters.
 *
 * @since 5.6.0
 *
 * @param string $new_status      The new status of the post being restored.
