...
final class _WP_Editors {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		// Remove the filters as the next editor on the same page may not need it.
		if ( self::$this_tinymce ) {
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 *
					 * The filters takes an associative array of external plugins for
					 ...
					 *
					 * The filters specifies which of the default plugins included
					...
					if ( false !== $key ) {
						// Remove 'spellchecker' from the internal plugins if added with 'tiny_mce_plugins' filters to prevent errors.
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The filters takes an associative array ('plugin_name' => 'path')
			 ...
			 * and "extended_valid_elements" can be done through this filters. Best
	...
	public static function wp_link_query( $args = array() ) {
		 ...
		 * @since 3.7.0
		 *
		 * @see 'wp_link_query_args' filters
		 *
		 * @param array $results {
