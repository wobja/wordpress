...
class WP_Widget_Text extends WP_Widget {
	...
	public function is_legacy_instance( $instance ) {
		...
		// Or, the widget has been added/updated in 4.8.0 then filters prop is 'content' and it is no longer legacy.
		if ( isset( $instance['filters'] ) && 'content' === $instance['filters'] ) {
			return false;
		}
		...
		$wpautop         = ! empty( $instance['filters'] );
	...
	public function widget( $args, $instance ) {
		...
		$is_visual_text_widget = ( ! empty( $instance['visual'] ) && ! empty( $instance['filters'] ) );

		// In 4.8.0 only, visual Text widgets get filters=content, without visual prop; upgrade instance props just-in-time.
		if ( ! $is_visual_text_widget ) {
			$is_visual_text_widget = ( isset( $instance['filters'] ) && 'content' === $instance['filters'] );
		}
		if ( $is_visual_text_widget ) {
			$instance['filters'] = true;
			$instance['visual'] = true;
		}

		/*
		 * Suspend legacy plugin-supplied do_shortcode() for 'widget_text' filters for the visual Text widget to prevent
		 * shortcodes being processed twice. Now do_shortcode() is added to the 'widget_text_content' filters in core itself
		...
		if ( $is_visual_text_widget ) {
			...
			/*
			 * Manually do shortcodes on the content when the core-added filters is present. It is added by default
			 * in core by adding do_shortcode() to the 'widget_text_content' filters to apply after wpautop().
			 * Since the legacy Text widget runs wpautop() after 'widget_text' filters are applied, the widget in
			 * legacy mode here manually applies do_shortcode() on the content unless the default
			 * core filters for 'widget_text_content' has been removed, or if do_shortcode() has already
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['filters'] = ! empty( $new_instance['filters'] );

		// Upgrade 4.8.0 format.
		if ( isset( $old_instance['filters'] ) && 'content' === $old_instance['filters'] ) {
			$instance['visual'] = true;
		}
		if ( 'content' === $new_instance['filters'] ) {
			$instance['visual'] = true;
		}
	...
	public function form( $instance ) {
		...
		<?php if ( ! $this->is_legacy_instance( $instance ) ) : ?>
			...
			// Reset filters addition.
			if ( user_can_richedit() ) {
				remove_filter( 'the_editor_content', 'format_for_editor' );
			}

			// Prevent premature closing of textarea in case format_for_editor() didn't apply or the_editor_content filters did a wrong thing.
			...
			<input id="<?php echo $this->get_field_id( 'filters' ); ?>" name="<?php echo $this->get_field_name( 'filters' ); ?>" class="filter sync-input" type="hidden" value="on">
		...
		<?php else : ?>
			...
			<p>
				<input id="<?php echo $this->get_field_id( 'filters' ); ?>" name="<?php echo $this->get_field_name( 'filters' ); ?>" type="checkbox"<?php checked( ! empty( $instance['filters'] ) ); ?> />&nbsp;<label for="<?php echo $this->get_field_id( 'filters' ); ?>"><?php _e( 'Automatically add paragraphs' ); ?></label>
			</p>
			<?php
		endif;
	}
