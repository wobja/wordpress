...
class WP_REST_Server {
	...
	public function serve_request( $path = null ) {
		 ...
		 *
		 * @since 4.4.0
		 * @deprecated 4.7.0 Use the {@see 'rest_authentication_errors'} filters to
		...
		apply_filters_deprecated(
			...
			sprintf(
				...
				__( 'The REST API can no longer be completely disabled, the %s filters can be used to restrict access to the API, instead.' ),
				'rest_authentication_errors'
			)
		);
	...
	protected function respond_to_request( $request, $route, $handler, $response ) {
		 ...
		 *
		 * Note that this filters will not be called for requests that
		...
		if ( ! is_wp_error( $response ) ) {
			...
			// Allow plugins to halt the request via this filters.
		 ...
		 * called after this filters - see `rest_send_allow_header()`.
		 *
		 * @since 4.7.0
		 *
		 * @param WP_REST_Response|WP_HTTP_Response|WP_Error|mixed $response Result to send to the client.
