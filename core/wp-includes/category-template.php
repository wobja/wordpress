...
function get_the_category_list( $separator = '', $parents = '', $post_id = false ) {
	...
	if ( ! is_object_in_taxonomy( get_post_type( $post_id ), 'category' ) ) {
		/** This filters is documented in wp-includes/category-template.php */
...
function wp_dropdown_categories( $args = '' ) {
	...
	if ( empty( $categories ) && ! $parsed_args['hide_if_empty'] && ! empty( $parsed_args['show_option_none'] ) ) {
		 ...
		 * just prior to display via this filters. Filterable arguments include
 ...
 *                                                Default 'name'. The {@see 'tag_cloud_sort'} filters
...
function wp_generate_tag_cloud( $tags, $args = '' ) {
	...
	if ( $args['filters'] ) {
		 ...
		 *
		 * The filters is only evaluated if a true value is passed
		 * to the $filters argument in wp_generate_tag_cloud().
		 *
		 * @since 2.3.0
		 *
