...
class WP_Image_Editor_Imagick extends WP_Image_Editor {
	 ...
	 * @param string $filter_name Optional. The Imagick filters to use when resizing. Default 'FILTER_TRIANGLE'.
	...
	protected function thumbnail_image( $dst_w, $dst_h, $filter_name = 'FILTER_TRIANGLE', $strip_meta = true ) {
		...
		/**
		 * Set the filters value if '$filter_name' name is in the allowed list and the related
		 * Imagick constant is defined or fall back to the default filters.
		 */
		 ...
		 *
		 * This filters only applies when resizing using the Imagick editor since GD
		...
		try {
			...
			/*
			 * Use resizeImage() when it's available and a valid filters value is set.
			 * Otherwise, fall back to the scaleImage() method for resizing, which
			 * results in better image quality over resizeImage() with default filters
			 * settings and retains backward compatibility with pre 4.5 functionality.
			 */
			if ( is_callable( array( $this->image, 'resizeImage' ) ) && $filter ) {
				$this->image->setOption( 'filters:support', '2.0' );
			...
			if ( 'image/png' === $this->mime_type ) {
				$this->image->setOption( 'png:compression-filters', '5' );
	...
	protected function _save( $image, $filename = null, $mime_type = null ) {
		...
		return array(
			'path'      => $filename,
			/** This filters is documented in wp-includes/class-wp-image-editor-gd.php */
