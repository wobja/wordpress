...
function search_theme_directories( $force = false ) {
	 ...
	 * @param string $context          The class or function name calling the filters.
	 */
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $wp_theme_directories, true ) ) {
			// Absolute path. Make an educated guess. YMMV -- but note the filters below.
			if ( 0 === strpos( $theme_root, WP_CONTENT_DIR ) ) {
 ...
 *
 * You can use the {@see 'validate_current_theme'} filters to return false to disable
 * this functionality.
 *
 * @since 1.5.0
 *
...
function get_theme_mod( $name, $default = false ) {
	...
	/** This filters is documented in wp-includes/theme.php */
...
function wp_update_custom_css_post( $css, $args = array() ) {
	 ...
	 *
	 * This filters can be used by plugin that offer CSS pre-processors, to store the original
	 * pre-processed CSS in `post_content_filtered` and then store processed CSS in `post_content`.
	 * When used in this way, the `post_content_filtered` should be supplied as the setting value
	 * instead of `post_content` via a the `customize_value_custom_css` filters, for example:
	 *
	 * <code>
...
function _wp_customize_include() {
	...
	// can be overridden via the customize_changeset_branching filters.
	$branching = false;
