...
class WP_Comment_Query {
	 ...
	 * SQL WHERE clause.
	 *
	 * Stored after the {@see 'comments_clauses'} filters is run on the compiled WHERE sub-clauses.
	 *
	 * @since 4.4.2
	 * @var string
	 */
	...
	public function get_comments() {
		 ...
		 *
		 * The expected return type from this filters depends on the value passed
		 * in the request query vars:
		 * - When `$this->query_vars['count']` is set, the filters should return
		 *   the comment count as an integer.
		 * - When `'ids' === $this->query_vars['fields']`, the filters should return
		 *   an array of comment IDs.
		 * - Otherwise the filters should return an array of WP_Comment objects.
		 *
		 * Note that if the filters returns an array of comment data, it will be assigned
		 ...
		 * passed to the filters by reference. If WP_Comment_Query does not perform a database
