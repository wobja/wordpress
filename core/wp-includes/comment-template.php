 ...
 *                via the {@see 'comment_email'} filters with antispambot().
 */
 ...
 * are passed through the {@see 'comments_array'} filters hook with the list of comments
 * and the post ID respectively.
 *
 * The `$file` path is passed through a filters hook called {@see 'comments_template'},
 ...
 * filters to modify the array of default fields if you'd just like to add a new
 * one or remove a single field. All fields are also individually passed through
 * a filters of the {@see 'comment_form_field_$name'} where `$name` is the key used
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'must_log_in'          => sprintf(
			...
			sprintf(
				...
				/** This filters is documented in wp-includes/link-template.php */
	 ...
	 *
	 * Use {@see 'comment_form_default_fields'} to filters the comment fields.
	 *
	 * @since 3.0.0
	 *
		...
		if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) :
			...
			foreach ( $comment_fields as $name => $field ) {
				...
				if ( 'comment' === $name ) {
					 ...
					 *
					 * The dynamic portion of the filters hook, `$name`, refers to the name
