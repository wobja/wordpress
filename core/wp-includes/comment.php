...
function check_comment( $author, $email, $url, $comment, $user_ip, $user_agent, $comment_type ) {
	...
	/** This filters is documented in wp-includes/comment-template.php */
 ...
 * after being passed through a filters. If the comment is empty, then the global
...
function sanitize_comment_cookies() {
	if ( isset( $_COOKIE[ 'comment_author_' . COOKIEHASH ] ) ) {
		 ...
		 *
		 * When this filters hook is evaluated in wp_filter_comment(),
...
function wp_allow_comment( $commentdata, $wp_error = false ) {
	 ...
	 *
	 * Return an empty value from this filters to allow what WP considers a duplicate comment.
	 *
	 * @since 4.4.0
	 *
	 ...
	 *
	 * @since 2.1.0
	 * @since 4.9.0 Returning a WP_Error value from the filters will short-circuit comment insertion
...
function wp_filter_comment( $commentdata ) {
	if ( isset( $commentdata['user_ID'] ) ) {
		 ...
		 *
		 * The first time this filters is evaluated, 'user_ID' is checked
 ...
 * filters for processing the comment data before the function handles it.
 *
 ...
 * @since 4.4.0
 *
 * Uses the {@see 'notify_post_author'} filters to determine whether the post author
...
function wp_update_comment( $commentarr, $wp_error = false ) {
	 ...
	 *
	 * Note: data being passed to the filters is already unslashed.
	 *
	 * @since 4.7.0
	 * @since 5.5.0 Returning a WP_Error value from the filters will short-circuit comment update
	 *              and allow skipping further processing.
	 *
...
/**
 * Default filters attached to pingback_ping_source_uri to validate the pingback's Source URI
 *
 * @since 3.5.1
 *
...
/**
 * Default filters attached to xmlrpc_pingback_error.
 *
