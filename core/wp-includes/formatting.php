...
function wptexturize( $text, $reset = false ) {
	...
	if ( $reset || ! isset( $static_characters ) ) {
		 ...
		 *
		 * Returning false from the filters will effectively short-circuit wptexturize()
		 * and return the original text passed to the function instead.
		 *
		 * The filters runs only once, the first time wptexturize() is called.
		 *
		 * @since 4.0.0
		 *
		 * @see wptexturize()
...
function sanitize_file_name( $filename ) {
	...
	if ( count( $parts ) <= 2 ) {
		/** This filters is documented in wp-includes/formatting.php */
 ...
 * parameters for the {@see 'sanitize_user'} filters.
 *
 * @since 2.0.0
 *
 ...
 * through the {@see 'sanitize_title'} filters.
 *
 ...
 * it is simply a holder for the {@see 'format_to_edit'} filters.
 *
 * @since 0.71
 ...
 *                          Default false.
 * @return string The text after the filters (and possibly htmlspecialchars()) has been run.
 */
...
function wp_rel_nofollow( $text ) {
	// This is a pre-save filters, so text is already escaped.
...
function is_email( $email, $deprecated = false ) {
	...
	if ( strlen( $email ) < 6 ) {
		 ...
		 *
		 * This filters is evaluated under several different contexts, such as 'email_too_short',
 ...
 * @since 1.5.0
 *
 * @param string $email Email address to filters.
...
function sanitize_email( $email ) {
	...
	if ( strlen( $email ) < 6 ) {
		 ...
		 *
		 * This filters is evaluated under several contexts, including 'email_too_short',
		 * 'email_no_at', 'local_invalid_chars', 'domain_period_sequence', 'domain_period_limits',
 ...
 *
 * The 55 word limit can be modified by plugins/themes using the {@see 'excerpt_length'} filters
 * The ' [&hellip;]' string can be modified by plugins/themes using the {@see 'excerpt_more'} filters
 *
 * @since 1.5.0
...
function wp_trim_excerpt( $text = '', $post = null ) {
	...
	if ( '' === trim( $text ) ) {
		...
		/** This filters is documented in wp-includes/post-template.php */
...
function ent2ncr( $text ) {
	 ...
	 *
	 * A non-null string must be returned for the filters to be evaluated.
	 *
	 * @since 3.3.0
	 *
 ...
 *
 * The filters {@see 'format_for_editor'} is applied here. If `$text` is empty the
 * filters will be applied to an empty string.
 *
 * @since 4.3.0
 *
 ...
 * @return string The formatted text after filters is applied.
 */
 ...
 * (the default behaviour) ampersands are also replaced. The {@see 'clean_url'} filters
 ...
 * @return string The cleaned URL after the {@see 'clean_url'} filters is applied.
 ...
 * be in single quotes. The {@see 'js_escape'} filters is also applied here.
 *
 * @since 2.8.0
 *
...
function wp_pre_kses_block_attributes( $string, $allowed_html, $allowed_protocols ) {
	 ...
	 * the filters to avoid recursion.
	 */
...
function wp_sprintf_l( $pattern, $args ) {
	 ...
	 * removed before the array of strings reaches the filters.
	 *
...
function _print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		$settings['source'] = array(
			/** This filters is documented in wp-includes/class.wp-scripts.php */
...
function wp_staticize_emoji_for_email( $mail ) {
	 ...
	 * then pass it through the wp_mail_content_type filters, in case a plugin
