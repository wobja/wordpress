...
class WP_Term_Query {
	...
	public function parse_query( $query = '' ) {
		 ...
		 *
		 * Use {@see 'get_terms_args'} to filters the passed arguments.
		 *
		 * @since 4.4.0
		 *
	...
	public function get_terms() {
		...
		if ( ! empty( $exclusions ) ) {
			// Must do string manipulation here for backward compatibility with filters.
		 ...
		 *
		 * Field lists modified using this filters will only modify the term fields returned
		 * by the function when the `$fields` parameter set to 'count' or 'all'. In all other
		 * cases, the term fields in the results array will be determined by the `$fields`
		 * parameter alone.
		 *
		 * Use of this filters can result in unpredictable behavior, and is not recommended.
		 *
		 * @since 2.8.0
		 *
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		// Run after the 'get_terms_orderby' filters for backward compatibility.
