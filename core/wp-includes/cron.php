 ...
 *              {@see 'pre_schedule_event'} filters added to short-circuit the function.
...
function wp_schedule_single_event( $timestamp, $hook, $args = array(), $wp_error = false ) {
	 ...
	 *
	 * Both single events and recurring events are passed through this filters;
	 ...
	 * filters to check if another plugin has disallowed the event before scheduling.
	 *
 ...
 * be extended using the {@see 'cron_schedules'} filters in wp_get_schedules().
 *
 * Note that scheduling an event to occur within 10 minutes of an existing event
 * with the same action hook will be ignored unless you pass unique `$args` values
 * for each scheduled event.
...
function wp_schedule_event( $timestamp, $recurrence, $hook, $args = array(), $wp_error = false ) {
	...
	/** This filters is documented in wp-includes/cron.php */
	$pre = apply_filters( 'pre_schedule_event', null, $event, $wp_error );
 ...
 * @since 2.1.0
 * @since 5.1.0 Return value modified to boolean indicating success or failure,
 *              {@see 'pre_reschedule_event'} filters added to short-circuit the function.
 * @since 5.7.0 The `$wp_error` parameter was added.
 *
 ...
 * @since 2.1.0
 * @since 5.1.0 Return value modified to boolean indicating success or failure,
 *              {@see 'pre_unschedule_event'} filters added to short-circuit the function.
 * @since 5.7.0 The `$wp_error` parameter was added.
 *
 ...
 *              {@see 'pre_clear_scheduled_hook'} filters added to short-circuit the function.
 * @since 5.7.0 The `$wp_error` parameter was added.
 *
 * @param string $hook     Action hook, the execution of which will be unscheduled.
 * @param array  $args     Optional. Array containing each separate argument to pass to the hook's callback function.
	...
	$cron_request = apply_filters(
		...
		array(
			...
			'args' => array(
				'timeout'   => 0.01,
				'blocking'  => false,
				/** This filters is documented in wp-includes/class-wp-http-streams.php */
 ...
 * A plugin may add more by hooking into the {@see 'cron_schedules'} filters.
 * The filters accepts an array of arrays. The outer array has a key that is the name
 ...
 *
 * @since 2.1.0
 * @since 5.1.0 {@see 'get_schedule'} filters added.
 *
