...
final class WP_Customize_Manager {
	 ...
	 *                                                 `customize_changeset_branching` filters returns false, then the default UUID will be that
	...
	public function __construct( $args = array() ) {
		 ...
		 * filtering them out of the array. Note that this filters generally runs
	 ...
	 *
	 * Check if customize query variable exist. Init filters to filters the current theme.
	 *
	 * @since 3.4.0
	 *
	 ...
	 *
	 * This method runs right at after_setup_theme and applies the 'customize_changeset_branching' filters to determine
	...
	/**
	 * If the theme to be previewed isn't the active theme, add filters callbacks
	...
	public function branching() {
		 ...
		 * by multiple users then branching changesets should be enabled by means of this filters.
		 *
		 * @since 4.9.0
		 *
	 ...
	 * @deprecated 4.7.0
	 *
	 * @param mixed $return Value passed through for {@see 'wp_die_handler'} filters.
	 * @return mixed Value passed through for {@see 'wp_die_handler'} filters.
	 */
	...
	public function validate_setting_values( $setting_values, $options = array() ) {
		...
		foreach ( $setting_values as $setting_id => $unsanitized_value ) {
			...
			if ( ! is_wp_error( $validity ) ) {
				/** This filters is documented in wp-includes/class-wp-customize-setting.php */
	...
	public function save() {
		 ...
		 *
		 * This filters does not apply if there was a nonce or authentication failure.
		 *
		 * @since 4.2.0
		 *
	...
	function save_changeset_post( $args = array() ) {
		 ...
		 *
		 * Plugins may amend additional data (such as additional meta for settings) into the changeset with this filters.
		 *
		 * @since 4.7.0
		 *
	 ...
	 *
	 * @since 4.9.0
	 * @param array $settings Current settings to filters.
	...
	public function add_setting( $id, $args = array() ) {
		if ( $id instanceof WP_Customize_Setting ) {
			...
			/** This filters is documented in wp-includes/class-wp-customize-manager.php */
	...
	public function add_dynamic_settings( $setting_ids ) {
		...
		foreach ( $setting_ids as $setting_id ) {
			 ...
			 *
			 * For a dynamic setting to be registered, this filters must be employed
	...
	public function remove_panel( $id ) {
		...
		if ( in_array( $id, $this->components, true ) ) {
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					/* translators: 1: Panel ID, 2: Link to 'customize_loaded_components' filters reference. */
					__( 'Removing %1$s manually will cause PHP warnings. Use the %2$s filters instead.' ),
					$id,
					sprintf(
						'<a href="%1$s">%2$s</a>',
	 ...
	 * using the customize_allowed_urls filters.
	 *
	 * @since 4.7.0
	 *
