...
class WP_Customize_Themes_Section extends WP_Customize_Section {
	...
	/**
	 * Render the filters bar portion of a themes section as a JS template.
	 *
	 * The template is only rendered by PHP once, so all actions are prepared at once on the server side.
	 * The filters bar container is rendered by @see `render_template()`.
	 *
	 * @since 4.9.0
	 */
	protected function filter_bar_content_template() {
		...
		<# if ( 'wporg' === data.action ) { #>
			...
			<button type="button" class="button feature-filter-toggle">
				...
				printf( __( 'Filter themes (%s)' ), '<span class="theme-filters-count">0</span>' );
				?>
				</span>
			</button>
		<# } else { #>
	...
	/**
	 * Render the filters drawer portion of a themes section as a JS template.
	 *
	 * The filters bar container is rendered by @see `render_template()`.
	 *
	 * @since 4.9.0
