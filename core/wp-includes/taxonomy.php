 ...
 * Must return term object. Used in get_term() as a catch-all filters for every
 * $term.
 *
 * {@see 'get_$taxonomy'} hook - Takes two parameters the term Object and the taxonomy
 * name. Must return term object. $taxonomy will be the taxonomy name, so for
 * example, if 'category', it would be 'get_category' as the filters name. Useful
 ...
 *
 * @see sanitize_term_field() The $context param lists the available values for get_term_by() $filters param.
 *
...
function get_term( $term, $taxonomy = '', $output = OBJECT, $filter = 'raw' ) {
	 ...
	 * Filters a taxonomy term object.
	 *
	 * The dynamic portion of the filters name, `$taxonomy`, refers
	...
	// Bail if a filters callback has changed the type of the `$_term` object.
	if ( ! ( $_term instanceof WP_Term ) ) {
		return $_term;
	}

	// Sanitize term, according to the specified filters.
	$_term->filter( $filter );
 ...
 * well as control the output with a filters.
 *
 ...
 *
 * The {@see 'get_terms'} filters will be called when the cache has the term and will
 * pass the found term along with the array of $taxonomies and array of $args.
 * This filters is also called before the array of terms is passed and will pass
 * the array of terms, along with the $taxonomies and $args.
 *
 * The {@see 'list_terms_exclusions'} filters passes the compiled exclusions along with
 * the $args.
 *
 * The {@see 'get_terms_orderby'} filters passes the `ORDER BY` clause for the query
 ...
 * without creating your own filters function. Simply create a function that
 * hooks into the filters you need.
 *
 * @since 2.3.0
 *
...
function sanitize_term_field( $field, $value, $term_id, $taxonomy, $context ) {
	...
	if ( 'edit' === $context ) {
		 ...
		 *
		 * The dynamic portion of the filters name, `$field`, refers to the term field.
		 *
		 * @since 2.3.0
		 *
		 ...
		 *
		 * The dynamic portions of the filters name, `$taxonomy` and `$field`, refer
		 ...
		 *
		 * The dynamic portion of the filters name, `$field`, refers to the term field name.
		 *
		 * @since 2.3.0
		 *
		 * @param mixed  $value    Value of the term field.
...
function wp_get_object_terms( $object_ids, $taxonomies, $args = array() ) {
	 ...
	 * Filters the terms for a given object or objects.
	 *
	 * The `$taxonomies` parameter passed to this filters is formatted as a SQL fragment. The
	 * {@see 'get_object_terms'} filters is recommended as an alternative.
	 *
	 * @since 2.8.0
	 *
	 * @param WP_Term[] $terms      Array of terms for the given object or objects.
...
function wp_insert_term( $term, $taxonomy, $args = array() ) {
	 ...
	 * to be created. Plugins with different uniqueness requirements may use this filters
...
function wp_update_term( $term_id, $taxonomy, $args = array() ) {
	 ...
	 *
	 * Hook to this filters to see if it will cause a hierarchy loop.
	 *
	 * @since 3.1.0
	 *
	...
	/** This filters is documented in wp-includes/taxonomy.php */
	$term_id = apply_filters( 'term_id_filter', $term_id, $tt_id );
...
function get_term_link( $term, $taxonomy = '' ) {
	...
	if ( 'post_tag' === $taxonomy ) {
		 ...
		 *
		 * @since 2.3.0
		 * @since 2.5.0 Deprecated in favor of {@see 'term_link'} filters.
 ...
 *
 * Attached to the {@see 'wp_update_term_parent'} filters.
 *
 * @since 3.1.0
 *
