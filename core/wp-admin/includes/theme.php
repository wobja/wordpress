 ...
 * @since 3.8.0 Renamed 'Width' filters to 'Layout'.
 ...
 * @since 4.6.0 Removed 'Colors' filters.
 ...
 *
 * The first filters, {@see 'themes_api_args'}, is for the args and gives the action
 * as the second parameter. The hook for {@see 'themes_api_args'} must ensure that
 * an object is returned.
 *
 * The second filters, {@see 'themes_api'}, allows a plugin to override the WordPress.org
 * Theme API entirely. If `$action` is 'query_themes', 'theme_information', or 'feature_list',
 * an object MUST be passed. If `$action` is 'hot_tags', an array should be passed.
 *
 * Finally, the third filters, {@see 'themes_api_result'}, makes it possible to filters the
 ...
 *     @type string  $tag      Tag to filters themes. Default empty.
 *     @type string  $author   Username of an author to filters themes. Default empty.
...
function themes_api( $action, $args = array() ) {
	 ...
	 *
	 * Important: An object MUST be returned to this filters.
	 *
	 * @since 2.8.0
	 *
...
function wp_prepare_themes_for_js( $themes = null ) {
	...
	foreach ( $themes as $theme ) {
		...
		if ( isset( $updates[ $slug ] ) ) {
			...
			/*
			 * Create the expected payload for the auto_update_theme filters, this is the same data
