...
function wp_edit_attachments_query_vars( $q = false ) {
	...
	$q['post_status'] = isset( $q['attachment-filters'] ) && 'trash' === $q['attachment-filters'] ? 'trash' : $states;
	...
	foreach ( array_keys( $post_mime_types ) as $type ) {
		if ( isset( $q['attachment-filters'] ) && "post_mime_type:$type" === $q['attachment-filters'] ) {
			$q['post_mime_type'] = $type;
			break;
		}
	}

	if ( isset( $q['detached'] ) || ( isset( $q['attachment-filters'] ) && 'detached' === $q['attachment-filters'] ) ) {
		$q['post_parent'] = 0;
	}

	if ( isset( $q['mine'] ) || ( isset( $q['attachment-filters'] ) && 'mine' === $q['attachment-filters'] ) ) {
...
function get_sample_permalink( $id, $title = null, $name = null ) {
	...
	if ( $ptype->hierarchical ) {
		...
		/** This filters is documented in wp-admin/edit-tag-form.php */
...
function wp_check_post_lock( $post_id ) {
	...
	/** This filters is documented in wp-admin/includes/ajax-actions.php */
...
function _admin_notice_post_locked() {
	...
	if ( $user ) {
		 ...
		 *
		 * Returning false from the filters will prevent the dialog from being displayed.
		 *
		 * @since 3.6.0
		 *
	...
	if ( $locked ) {
		 ...
		 *
		 * Returning false from the filters will disable the ability
...
function the_block_editor_meta_box_post_form_hidden_fields( $post ) {
	 ...
	 */
	ob_start();
	/** This filters is documented in wp-admin/edit-form-advanced.php */
