...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		if ( ! is_multisite() || ( $screen->in_admin( 'network' ) && current_user_can( 'manage_network_plugins' ) ) ) {
			 ...
			 * plugins to filters the display of. Contexts include 'mustuse' and 'dropins'.
			 *
			 * @since 3.0.0
			 *
		...
		foreach ( (array) $plugins['all'] as $plugin_file => $plugin_data ) {
			...
			/*
			 * Create the payload that's used for the auto_update_plugin filters.
			...
			if ( is_multisite() && ! $screen->in_admin( 'network' ) && is_network_only_plugin( $plugin_file ) && ! is_plugin_active( $plugin_file ) ) {
				if ( $show_network_active ) {
					...
					// On the non-network screen, filters out network-only plugins as long as they're not individually active.
				...
				if ( $show_network_active ) {
					...
					// On the non-network screen, filters out network-active plugins.
					unset( $plugins['all'][ $plugin_file ] );
				}
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				...
				case 'description':
					 ...
					 * @param string   $status      Status filters currently applied to the plugin list. Possible
