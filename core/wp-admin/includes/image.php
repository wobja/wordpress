...
function wp_update_image_subsizes( $attachment_id ) {
	...
	/** This filters is documented in wp-admin/includes/image.php */
...
function wp_create_image_subsizes( $file, $attachment_id ) {
	...
	if ( 'image/png' !== $imagesize['mime'] ) {
		 ...
		 *
		 * Returning `false` from the filters callback will disable the scaling.
		 *
		 * @since 5.3.0
		 *
	...
	if ( $filepath && file_exists( $filepath ) ) {
		if ( 'full' !== $size ) {
			...
			if ( $data ) {
				 ...
				 *
				 * The filters is evaluated for all image sizes except 'full'.
				 *
				 * @since 3.1.0
				 *
		 ...
		 *
		 * The filters is only evaluated if the file isn't stored locally and `allow_url_fopen` is enabled on the server.
		 *
		 * @since 3.1.0
		 *
