...
class WP_Media_List_Table extends WP_List_Table {
	...
	public function __construct( $args = array() ) {
		$this->detached = ( isset( $_REQUEST['attachment-filters'] ) && 'detached' === $_REQUEST['attachment-filters'] );
	...
	public function prepare_items() {
		...
		$this->is_trash = isset( $_REQUEST['attachment-filters'] ) && 'trash' === $_REQUEST['attachment-filters'];
	...
	protected function get_views() {
		...
		$filter = empty( $_GET['attachment-filters'] ) ? '' : $_GET['attachment-filters'];
	...
	/**
	 * Override parent views so we can use the filters bar display.
	 *
	 * @global string $mode List table view mode.
	 */
	public function views() {
		...
		<div class="wp-filter">
			<div class="filter-items">
				...
				/** This filters is documented in wp-admin/inclues/class-wp-list-table.php */
				$views = apply_filters( "views_{$this->screen->id}", array() );

				// Back compat for pre-4.0 view links.
				if ( ! empty( $views ) ) {
					echo '<ul class="filters-links">';
					foreach ( $views as $class => $view ) {
	...
	public function column_default( $post, $column_name ) {
		 ...
		 *
		 * Custom columns are registered using the {@see 'manage_media_columns'} filters.
		 *
		 * @since 2.5.0
		 *
