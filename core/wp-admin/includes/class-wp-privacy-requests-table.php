...
abstract class WP_Privacy_Requests_Table extends WP_List_Table {
	...
	protected function get_views() {
		$current_status = isset( $_REQUEST['filters-status'] ) ? sanitize_text_field( $_REQUEST['filters-status'] ) : '';
		...
		foreach ( $statuses as $status => $label ) {
			...
			$status_link = add_query_arg( 'filters-status', $status, $admin_url );
	...
	public function prepare_items() {
		...
		if ( ! empty( $_REQUEST['filters-status'] ) ) {
			$filter_status       = isset( $_REQUEST['filters-status'] ) ? sanitize_text_field( $_REQUEST['filters-status'] ) : '';
