...
(function( wp, $ ){
	...
	api.Widgets.AvailableWidgetsPanelView = wp.Backbone.View.extend(/** @lends wp.customize.Widgets.AvailableWidgetsPanelView.prototype */{
		...
		search: _.debounce( function( event ) {
			...
			// If a widget was selected but the filters value has been cleared out, clear selection.
			if ( this.selected && ! event.target.value ) {
				this.selected.removeClass( 'selected' );
				this.selected = null;
			}

			// If a filters has been entered and a widget hasn't been selected, select the first one shown.
			if ( ! this.selected && event.target.value ) {
