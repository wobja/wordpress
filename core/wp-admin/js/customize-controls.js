	...
	api.Section = Container.extend(/** @lends wp.customize.Section.prototype */{
		...
		attachEvents: function () {
			...
			section.container.find( '.accordion-section-title, .customize-section-back' ).on( 'click keydown', function( event ) {
				...
				event.preventDefault(); // Keep this AFTER the key filters above.
	...
	api.ThemesSection = api.Section.extend(/** @lends wp.customize.ThemesSection.prototype */{
		...
		attachEvents: function () {
			...
			section.headerContainer.on( 'click', '.customize-themes-section-title', function() {
				...
				if ( section.headerContainer.find( '.filters-details' ).length ) {
					section.headerContainer.find( '.customize-themes-section-title' )
						.toggleClass( 'details-open' )
						.attr( 'aria-expanded', function( i, attr ) {
							return 'true' === attr ? 'false' : 'true';
						});
					section.headerContainer.find( '.filters-details' ).slideToggle( 180 );
				}
			...
			if ( 'local' === section.params.filter_type ) {
				...
				section.container.on( 'input', '.wp-filters-search-themes', function( event ) {
				...
				section.contentContainer.on( 'input', '.wp-filters-search', function() {
				...
				// Feature filters.
				section.contentContainer.on( 'click', '.filters-group input', function() {
			...
			section.contentContainer.on( 'click', '.feature-filters-toggle', function( e ) {
				var $themeContainer = $( '.customize-themes-full-container' ),
					$filterToggle = $( e.currentTarget );
				section.filtersHeight = $filterToggle.parent().next( '.filters-drawer' ).height();
				...
				$filterToggle
					...
					.parent().next( '.filters-drawer' ).slideToggle( 180, 'linear' );
		...
		onChangeExpanded: function ( expanded, args ) {
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'themes' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.wp-filters-search' ).val();
							section.contentContainer.find( '.wp-filters-search' ).val( searchTerm );
			...
			if ( expanded ) {
				...
				section.headerContainer.find( '.filters-details' ).slideUp( 180 );
		...
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.wp-filters-search' ).val();
		...
		/**
		 * Check for filters checked in the feature filters list and initialize a new query.
		 *
		 * @since 4.9.0
		 *
		 * @return {void}
		...
		filtersChecked: function() {
			var section = this,
			    items = section.container.find( '.filters-group' ).find( ':checkbox' ),
			    tags = [];
			...
			// When no filters are checked, restore initial state. Update filters count.
			if ( 0 === tags.length ) {
				tags = '';
				section.contentContainer.find( '.feature-filters-toggle .filters-count-0' ).show();
				section.contentContainer.find( '.feature-filters-toggle .filters-count-filters' ).hide();
			} else {
				section.contentContainer.find( '.feature-filters-toggle .theme-filters-count' ).text( tags.length );
				section.contentContainer.find( '.feature-filters-toggle .filters-count-0' ).hide();
				section.contentContainer.find( '.feature-filters-toggle .filters-count-filters' ).show();
			}
