...
themes.view.Appearance = wp.Backbone.View.extend({
	...
	search: function() {
		...
		this.searchContainer
			.append( $.parseHTML( '<label class="screen-reader-text" for="wp-filters-search-input">' + l10n.search + '</label>' ) )
			.append( view.el )
...
themes.view.Search = wp.Backbone.View.extend({
	...
	tagName: 'input',
	className: 'wp-filters-search',
	id: 'wp-filters-search-input',
	searching: false,
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.wp-filters-search' ).val( query.replace( /\+/g, ' ' ) );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.wp-filters-search' ).trigger( 'keyup' );
		});
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	render: function() {
		...
		this.listenTo( this.collection, 'query:fail', function() {
			...
			$( '.theme-browser .error .try-again' ).on( 'click', function( e ) {
				e.preventDefault();
				$( 'input.wp-filters-search' ).trigger( 'input' );
			} );
		});
	...
	sort: function( sort ) {
		...
		$( '.filters-links li > a, .theme-filters' )
	...
	onFilter: function( event ) {
		...
		$( '.filters-links li > a, .theme-section' )
			.removeClass( this.activeClass )
			.removeAttr( 'aria-current' );
		$el
			.addClass( this.activeClass )
	...
	// Clicking on a checkbox to add another filters to the request.
	addFilter: function() {
		this.filtersChecked();
	},
	...
	applyFilters: function( event ) {
		...
		_.each( tags, function( tag ) {
			name = $( 'label[for="filters-id-' + tag + '"]' ).text();
	...
	filtersChecked: function() {
		var items = $( '.filters-group' ).find( ':checkbox' ),
			tags = [];
		...
		if ( tags.length === 0 ) {
			$( '.filters-drawer .apply-filters' ).find( 'span' ).text( '' );
			$( '.filters-drawer .clear-filters' ).hide();
			$( 'body' ).removeClass( 'filters-applied' );
			return false;
		}

		$( '.filters-drawer .apply-filters' ).find( 'span' ).text( tags.length );
		$( '.filters-drawer .clear-filters' ).css( 'display', 'inline-block' );
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.wp-filters-search' ).trigger( 'focus' ).trigger( 'keyup' );
		});
